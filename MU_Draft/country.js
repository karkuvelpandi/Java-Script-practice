...
const regionCountryCurrencyMap = [
  {
    region: "africa",
    country_code: "ZA",
    country_id: "ZAF",
...
const flagMissedData = regionCountryCurrencyMap
  .map((country) => {
    if (country.currency_flag === "" || !country?.currency_flag)
      return country.country_code.toLowerCase();
  })
  .filter((code) => code !== undefined);

console.log(flagMissedData);
console.log(flagMissedData.length);

const flagPrifile = countryProfile.filter((country) =>
  flagMissedData.includes(country.countryCode.toLowerCase())
);

console.log(flagPrifile.length);

const finalData = regionCountryCurrencyMap.map((country) => {
  if (country.currency_flag == "" || !country.currency_flag) {
    const flag = flagPrifile.filter(
      (profile) => profile.countryCode === country.country_code.toLowerCase()
    );
    if (flag.length > 0) {
      country.currency_flag = flag[0].flag;
      return country;
    } else return country;
  } else {
    return country;
...
console.log("Initial data length", regionCountryCurrencyMap.length);
console.log("Final data length", finalData.length);

console.log(finalData);

const emptyOnfinaldata = finalData
  .map((country) => {
    if (country.currency_flag === "" || !country?.currency_flag)
...
console.log(emptyOnfinaldata);
console.log(emptyOnfinaldata.length);

console.log(finalData);

console.log(countryProfile.length);
